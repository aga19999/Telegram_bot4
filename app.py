...
# Se l'utente dopo il reminder non compila il questionario, esaurisce la possibilità di farlo
...
async def fetch_elenco_sessioni(context: ContextTypes.DEFAULT_TYPE) -> None:
    with connection.cursor() as cursor:
        ...
        for sessione in elenco_sessioni:
            context.job_queue.run_once(invia_questionario, (sessione["dataInvio"] - timedelta(hours=2)),
                                       data=sessione)
            # Aggiorna lo status della sessione per evitare che venga messa in coda più volte
