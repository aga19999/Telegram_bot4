...
async def invia_questionario_due(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    info_messaggio = (await context.bot.send_message(chat_id=id_utente,
                                   text="Ciao! Ho un sondaggio per te!",
                                   reply_markup=reply_markup))
    message_id = info_messaggio["message_id"]
    data.update({'message_id': message_id})
    #context.job_queue.run_once(reminder_utente, now + timedelta(seconds=40), data=data)
    print(f"Invio il questionario a {data["telegram_user_id"]} - Link: {data["survey"]}")
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if rilevazione is None:
                ...
                #await context.bot.delete_message(chat_id=id_utente, message_id=data["message_id"])
                kb = [
                    [InlineKeyboardButton("Clicca qui!", web_app=WebAppInfo(url))]
                ]
                reply_markup = InlineKeyboardMarkup(kb)
...
async def fetch_elenco_sessioni(context: ContextTypes.DEFAULT_TYPE) -> None:
    with connection.cursor() as cursor:
        ...
        for sessione in elenco_sessioni:
            context.job_queue.run_once(invia_questionario_due, (sessione["dataInvio"] - timedelta(hours=2)),
                                       data=sessione)
